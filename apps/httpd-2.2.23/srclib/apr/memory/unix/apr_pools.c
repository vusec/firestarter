...
int magic_mempool_allow_reset = 1;
int magic_mempool_allow_reuse = 1;
/* It needs to be 1 since the allocators allocate memory outside pool management and
 * reuse it afterwards as pool buffers. */
int magic_mempool_allow_external_alloc = 1;
...
apr_memnode_t *allocator_alloc(apr_allocator_t *allocator, apr_size_t in_size)
{
    ...
    /* if memory reuse across pools has been disabled by the application,
     * allocate a new node directly */
    if (!magic_mempool_allow_reuse) {
        goto no_reuse;
    }
...
static inline int env_parse_int(const char *env_name,
    int default_value)
{
...
APR_DECLARE(apr_status_t) apr_pool_initialize(void)
{
    ...
    magic_mempool_allow_reset = env_parse_int("MAGIC_MEMPOOL_ALLOW_RESET", 1);
    magic_mempool_allow_reuse = env_parse_int("MAGIC_MEMPOOL_ALLOW_REUSE", 1);
...
APR_DECLARE(void) apr_pool_clear(apr_pool_t *pool)
{
    ...
    if (magic_mempool_allow_reset) {
        ...
        *active->ref = NULL;
		...
		// free all the nodes and create a new one (similar to pool creation)
        active = pool->self;
        *active->ref = NULL;
        allocator_free(pool->allocator, active);
        if ((node = allocator_alloc(pool->allocator,
...
APR_DECLARE(void) apr_pool_destroy(apr_pool_t *pool)
{
    ...
    *active->ref = NULL;
    ...
    /* When reset is disabled by the application, the pool structure is held by a separate memory buffer */
    /*if (!magic_mempool_allow_reset) {
        free(pool);
    }*/
}
                                             ...
                                             apr_allocator_t *allocator)
{
    ...
    *newpool = NULL;
    ...
    } else { /* when reset is disabled, get a separate allocation for the pool structure
              * so that all the other nodes can be easily deallocated. */
/*        if ((pool = malloc(SIZEOF_POOL_T)) == NULL) {
        ...
        pool = (apr_pool_t *)node->first_avail;
        node->first_avail = pool->self_first_avail = node->endp;
    }

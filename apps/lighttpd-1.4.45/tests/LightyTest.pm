...
our $edfi_bbtracing = defined $ENV{EDFI_BBTRACING} ? $ENV{EDFI_BBTRACING} : 0;
our $sigs_toutin = defined $ENV{SIGTOUTIN} ? $ENV{SIGTOUTIN} : 1;
...
#	diag(sprintf("\nstop_proc called!\n"));
	...
	if (defined $pid && $pid != -1) {
		if ($edfi_bbtracing) {
			if ($sigs_toutin) {
	                	diag(sprintf("\nSending signal TTIN to dump BBTracing memory (pid:%d).\n", $pid));
	                        kill 'TTIN', $pid;
        	        } else {
                		diag(sprintf("\nSending signal PROF to dump BBTracing memory (pid:%d).\n", $pid));
                	        kill 'PROF', $pid;
	                }
			sleep 2;
        	}
                diag(sprintf("\nSending signal TERM to terminate pid:%d.\n", $pid));
...
#	diag(sprintf("\nstart_proc called!\n"));
	...
	diag(sprintf("\nThe child process is up: pid:%d\n", $child));
	$self->{LIGHTTPD_PID} = $child;

	my $pid = $self->{LIGHTTPD_PID};
	my $prompt;
	if (defined $ENV{"GDB_ATTACH"} && $ENV{"GDB_ATTACH"} eq '1') {
		diag(sprintf("lighttpd pid is: %d. gdb --attach to the process and press any key to continue.\n", $pid));
		$prompt = <STDIN>;
	}
	#diag(sprintf("\nEDFI_BBTRACING: %s, SIGTOUTIN: %s\n", $ENV{"EDFI_BBTRACING"}, $ENV{"SIGTOUTIN"}));
	if (defined $ENV{"EDFI_BBTRACING"} && $ENV{"EDFI_BBTRACING"} eq '1') {
        	if (defined $ENV{"SIGTOUTIN"} && $ENV{"SIGTOUTIN"} eq '0') {
                	diag(sprintf("\nSending signal WINCH to reset BBTracing memory (pid:%d).\n", $pid));
	                kill 'WINCH', $pid;
        	} else {
                	diag(sprintf("\nSending signal TTOU to reset BBTracing memory (pid:%d).\n", $pid));
	                kill 'TTOU', $pid;
        	}
                sleep 1;
    	}

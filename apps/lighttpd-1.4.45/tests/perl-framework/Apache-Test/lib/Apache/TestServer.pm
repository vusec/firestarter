...
sub stop {
    ...
    print "Stopping httpd Apache server.\n\n";
    if (WIN32) {
...
sub start {
    ...
    print "Starting httpd Apache server.\n\n";
    my $old_pid = -1;
    if (WIN32) {
...
EOI
    ...
    if ($edfi_bbtracing) {
	if ($sigs_toutin) {
		print "Sending signal TTOU to reset BBTracing memory (pid:%d).\n", $self->pid;
	        kill 'TTOU', $self->pid;
	} else {
		print "Sending signal WINCH to reset BBTracing memory (pid:%d).\n", $self->pid;
	        kill 'WINCH', $self->pid;
	}
		sleep 5;
    }

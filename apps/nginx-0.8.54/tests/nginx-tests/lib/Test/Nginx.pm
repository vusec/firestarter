...
our @EXPORT = qw/ log_in log_out http http_get http_head http_url /;
...
sub new {
	...
	if ($ENV{TEST_NGINX_TESTDIR}) {
		$self->{_testdir} = $ENV{TEST_NGINX_TESTDIR};
		mkdir($self->{_testdir});
	} else {
	...
	my @testnames = split(/\./, $0);
	$self->{_testname} = $testnames[0];
	$self->{_testprefix} = '';
	if ($ENV{TEST_NGINX_PREFIXED}) {
		$self->{_testprefix} = '/' . $self->{_testname};
	}
	$self->{_testprefixeddir} = $self->{_testdir} . $self->{_testprefix};
	mkdir($self->{_testprefixeddir});
...
sub DESTROY {
	...
	if ($ENV{TEST_NGINX_DUMP_CONF}) {
		system("cp $self->{_testdir}/nginx.conf $self->{_testname}.nginx.conf");
	}
	if (not $ENV{TEST_NGINX_LEAVE} and not $ENV{TEST_NGINX_TESTDIR}) {
		system("rm -rf $self->{_testdir}");
	}
	else {
		system("cp -f $self->{_testdir}/error.log $self->{_testdir}/$self->{_testname}.error.log");
		system("echo > $self->{_testdir}/error.log");
	}
}
...
sub write_file_from_dir($$$) {
	my ($self, $dir, $name, $content) = @_;
...
sub write_file($$) {
	...
	$content =~ s/include virtual=\"/include virtual=\"$self->{_testprefix}/gms;

	return $self->write_file_from_dir($self->{_testprefixeddir}, $name, $content);
}

sub write_file_expand($$) {
	my ($self, $name, $content) = @_;

	if ($name eq 'nginx.conf' and $ENV{TEST_NGINX_FORCE_CONF}) {
		$content = `cat $ENV{TEST_NGINX_FORCE_CONF}`;
		$self->test_globals();
		$self->test_globals_http();
	}
	else {
		$content =~ s/(location.*?) \//$1 $self->{_testprefix}\//gms;
		$content =~ s/(Redirect *) \//$1 $self->{_testprefix}\//gms;
		$content =~ s/(error_page.*?) \//$1 $self->{_testprefix}\//gms;
		$content =~ s/http:\/\/127.0.0.1:8080\//http:\/\/127.0.0.1:8080$self->{_testprefix}\//gms;
	}
	$content =~ s/%%TEST_GLOBALS%%/$self->test_globals()/gmse;
	$content =~ s/%%TEST_GLOBALS_HTTP%%/$self->test_globals_http()/gmse;
	$content =~ s/%%ROOTDIR%%/$self->{_testdir}/gms;
	$content =~ s/%%TESTDIR%%/$self->{_testprefixeddir}/gms;

	return $self->write_file_from_dir($self->{_testdir}, $name, $content);
}
...
sub http_url {
	my ($url) = @_;
	return $NGINX_TEST->{_testprefix} . $url;
}

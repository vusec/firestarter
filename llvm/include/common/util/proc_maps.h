    ...
    util_proc_maps_parse_cb_t cb, void* cb_args)
{
    ...
    while (!stop && bufread(fd, buf, sizeof(buf), &iobuf) > 0) {
        ...
        if (num_entries+1 > maps->num_entries) {
            int new_num_entries = (maps->num_entries+1)*2;
#ifdef USE_DELTA_HEAP_FUNCS
            delta_gss_lock();
            maps->entries = delta_orealloc(maps->entries,
                sizeof(util_proc_maps_entry_t)*new_num_entries,
            ...
            assert(maps->entries);
#endif
            maps->num_entries = new_num_entries;
        }
...
out:
    ...
    int __attribute__((unused)) old_num_entries = maps->num_entries;
    ...
    maps->entries = delta_orealloc(maps->entries,
        sizeof(util_proc_maps_entry_t)*maps->num_entries,
        sizeof(util_proc_maps_entry_t)*old_num_entries);

    if (ret != 0 && maps->entries)
        delta_free(maps->entries);

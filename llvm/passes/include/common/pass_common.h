...
#define SET_SOME_DEBUG_LOC(I, P) do { \
	const DebugLoc *dbgLoc = NULL;				\
        Function *inFunc = P->getParent()->getParent();		\
        if (NULL != P) dbgLoc = &(P->getDebugLoc());		\
        for (inst_iterator it = inst_begin(inFunc), et = inst_end(inFunc); \
	     it != et && (!dbgLoc || !dbgLoc->get()); it++) {		   \
               Instruction *ii = &(*it);				   \
                dbgLoc = &(ii->getDebugLoc());				   \
        }								   \
...
#define PREV_BB(B)              ({              \
        Function::iterator FI(B);               \
        if (FI == B->getParent()->begin()) {    \
            NULL;                               \
        }                                       \
        &*--FI;                                 \
        })
...
class PassUtil {
  public:
      ...
      static uint64_t getAssignedID(Value *V, std::string metadataNamespace, MDNode **mdNode=NULL);
      static uint64_t getNextUnassignedID(Module *M, std::string metadataNamespace, std::string lastIDHolderSuffix="_LAST_ID_HOLDER");
      static unsigned insertHookForEvery(Module &M, std::vector<Value*> *elementsList, Function *funcHook, std::vector<Value*> *args, std::vector<CallInst*> &output, bool insertBefore=true);
      static bool callHasNoUsers(CallInst *CI, std::vector<User *> &users);
      static void getUsers(Instruction *I, std::vector<User *> &users);
      static bool isLibCallInst(CallInst *CI);
      static bool isInAnyOfSections(Function &F, std::vector<std::string> &sectionsList);
      static void setBranchWeight(Module &M, BranchInst *branch, uint32_t prob_true, uint32_t prob_false);
  private:
      static Module *M;
};
...
inline Value *PassUtil::findDbgGlobalDeclare(GlobalVariable *V) {
  ...
  for (unsigned i = 0, e = GVs.size(); i != e; ++i)
    return GVs[i]->getVariable();
  return 0;
#elif LLVM_VERSION >= 30
    ...
    while(!N) {
        if (BI->isTerminator()) {
            ...
            if (BInst && BInst->isUnconditional()) {
                N = BInst->getSuccessor(0)->front().getMetadata("dbg");
                continue;
            }
            return false;
        }
...
#endif
#if LLVM_VERSION >= 40
    return type.getBaseType().resolve();
...
inline CallInst* PassUtil::createCallInstruction(Value *F, std::vector<Value*> &args, const Twine &NameStr, Instruction *InsertBefore) {
    ...
    CallInst *CI = CallInst::Create(F, ref, NameStr, InsertBefore);
    if (NULL != CI) {
	SET_SOME_DEBUG_LOC(CI, InsertBefore);
    }
    return CI;
#elif LLVM_VERSION >= 30
    ArrayRef<Value*> ref(args);
...
#if LLVM_VERSION >= 40
    ArrayRef<Value*> ref(args);
    CallInst *CI = CallInst::Create(F, ref, NameStr, InsertAtEnd);
    if (NULL != CI) {
	Instruction *I = NULL;
	SET_SOME_DEBUG_LOC(CI, I);
    }
    return CI;
#elif LLVM_VERSION >= 30
    ArrayRef<Value*> ref(args);
...
inline MDNode* PassUtil::createMDNodeForConstant(Module *M, uint64_t constantValue)
{
  IntegerType *T = IntegerType::get(M->getContext(), 64);
  ConstantInt *I = ConstantInt::get(T, constantValue);
#if LLVM_VERSION >= 40
  MDNode *N = MDNode::get(M->getContext(), ConstantAsMetadata::get(I));
...
inline uint64_t PassUtil::assignIDs(Module &M, std::vector<Value*> *elementsList, std::string metadataNamespace, std::string lastIDHolderSuffix, uint64_t idStart, bool forceReset)
{
...
inline uint64_t PassUtil::getAssignedID(Value *V, std::string metadataNamespace, MDNode **mdNode)
{
  ...
  errs() << "Warning: Failed getting assigned ID.\n";
  return 0;
}

inline uint64_t PassUtil::getNextUnassignedID(Module *M, std::string metadataNamespace, std::string lastIDHolderSuffix)
{
...
/* Returns idStart on Failure and last assigned valid value upon success. */
inline unsigned PassUtil::insertHookForEvery(Module &M, std::vector<Value*> *elementsList, Function *funcHook, std::vector<Value*> *args, std::vector<CallInst*> &output, bool insertBefore)
{
  ...
  for (std::vector<Value*>::iterator IV=elementsList->begin(), EV=elementsList->end(); IV != EV; IV++)
  {
    ...
    if (NULL != currFunction)
    {
      ...
      targetInst = currFunction->getEntryBlock().getFirstNonPHI();
      insertBefore = true; // We cannot insert anything "after" a function! (except, maybe at every return points; not for now)
    }
    if (NULL != currBB)
    {
        targetInst = currBB->getFirstNonPHI();
    }
    assert(NULL != targetInst && "Invalid element in elementsList");
    if (!insertBefore) {
	targetInst = NEXT_INST(targetInst);
	assert(NULL != targetInst && "Failed fetching next instruction.");
    }
    CallInst *CI = PassUtil::createCallInstruction(funcHook, *args, "call " + funcHook->getName(), targetInst);
    assert (NULL == CI && "Couldn't create call instruction.");
    output.push_back(CI);
    num_inserts++;
  }
  return num_inserts;
...
inline bool PassUtil::callHasNoUsers(CallInst *CI, std::vector<User *> &users)
{
    assert(NULL != CI);
    ...
    // Get all users
#if LLVM_VERSION >= 40
    users.assign(CI->user_begin(), CI->user_end());
#else
    users.assign(CI->use_begin(), CI->use_end());
#endif
    if (0 == users.size()) {
        return true;
    }
...
inline void PassUtil::getUsers(Instruction *I, std::vector<User *> &users)
{
    assert(NULL != I);
    // Get all users
#if LLVM_VERSION >= 40
...
inline bool PassUtil::isLibCallInst(CallInst *CI)
{
    assert(NULL != CI);
    Function *F = CI->getCalledFunction();
    /* Is F an extern func? */
    if ((NULL != F) && (!F->isIntrinsic())
        && (F->isDeclaration() || (F->begin() == F->end()))) {
        return true;
    }
    return false;
}

inline bool PassUtil::isInAnyOfSections(Function &F, std::vector<std::string> &sectionsList)
{
...
inline void PassUtil::setBranchWeight(Module &M, BranchInst *branch, uint32_t prob_true, uint32_t prob_false)
{
